 ...
 *
 * 10 Jan 2005 - added '@name,[cubp],maj,min' special files
...
/*
 * Look for a filename pattern to make dev nodes as non-root user.  The file
 * must match the pattern '@name,[cubp],major,minor' and have 0 length
 */
static int
special_file(char *filename, char **dir_name, struct stat *buf)
{
	char *bname;
	static char dev[32];
	char typ;
	int maj, min;
#define NODE_PATTERN "@%[a-zA-Z0-9_+-],%c,%d,%d"
	...
	if (!bname)
		...
		bname++;
	if (sscanf(bname, NODE_PATTERN, dev, &typ, &maj, &min) == 4) {
		if (buf) {
			if (!S_ISREG(buf->st_mode)) {
				fprintf(stderr, "Special pattern found on non-file: %s\n",
						filename);
				return -1;
			}
			if (buf->st_size != 0) {
				fprintf(stderr, "Special pattern file with size: %s\n",
						filename);
				return -1;
			}
...
int create_inode(squashfs_inode *i_no, char *filename, int type, int byte_size, squashfs_block start_block, unsigned int offset, unsigned int *block_list, struct fragment *fragment, struct cached_dir_index *index, unsigned int i_count, unsigned int i_size)
{
	...
	if(filename[0] == '\0') {
		...
		perror(buffer);
		return FALSE;
	} else if(special_file(filename, NULL, &buf) == -1)
		return FALSE;
...
int write_file(squashfs_inode *inode, char *filename, long long size, int *duplicate_file)
{
	...
	if((file = open(filename, O_RDONLY)) == -1) {
		fprintf(stderr, "Error in opening file '%s', skipping...", filename);
		return FALSE;
	}
...
int compare_name(const void *ent1_ptr, const void *ent2_ptr)
{
	...
	if (special_file(ent1->name, &name1, NULL) != -1 && name1)
		name1 = strdup(name1);

	if (special_file(ent2->name, &name2, NULL) != -1 && name2)
		name2 = strdup(name2);

	rc = strcmp(name1 ? name1 : ent1->name, name2 ? name2 : ent2->name);
	if (name1)
		free(name1);
	if (name2)
		free(name2);
...
int dir_scan(squashfs_inode *inode, char *pathname, int (_opendir)(char *, struct directory *))
{
	...
	while(linux_readdir(&dir, &filename, &dir_name) != FALSE) {
		...
		if(special_file(filename, &dir_name, &buf) == -1)
			continue;
...
printOptions:
	...
	/* force little endian for cramfs */
	sBlk.cramfs_magic[0] = 0x45;
	sBlk.cramfs_magic[1] = 0x3d;
	sBlk.cramfs_magic[2] = 0xcd;
	sBlk.cramfs_magic[3] = 0x28;
#endif
...
#ifdef CONFIG_SQUASHFS_CRAMFS_MAGIC
{
	unsigned int cs = ((bytes - 1) | (4096 - 1)) + 1;
	/* always force little endian for cramfs */
	sBlk.cramfs_size[0] = ((cs >> 0 ) & 0xff);
	sBlk.cramfs_size[1] = ((cs >> 8 ) & 0xff);
	sBlk.cramfs_size[2] = ((cs >> 16) & 0xff);
	sBlk.cramfs_size[3] = ((cs >> 24) & 0xff);
}
#endif
